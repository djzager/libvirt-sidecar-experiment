...
if [ -z $PROVISIONING_IF ]; then
  echo "Must specify provisioning interface"
  exit 1
fi

if [ -z $BAREMETAL_IF ]; then
  echo "Must specify baremetal interface"
  exit 1
fi

cleanup() {
  ip link del provisioning
  ip link del baremetal
}
...
ip link add provisioning type bridge
ip link set $PROVISIONING_IF master provisioning
ip address add 172.22.0.101/24 dev provisioning
...
ip link add baremetal type bridge
ip link set $BAREMETAL_IF master baremetal
